...
const ProjectsArchives: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchProjects = async () => {
            await axios.get(`https://localhost:4000/ProjectArchives/Archives`, {
    ...
    const handleDelete= (id: any) => {
        axios.delete(`https://localhost:4000/ProjectArchives/${id}`, {
            headers: { authorization: Cookie.auth_token }
        })
        .then(() => { 
    ...
    const handleArchive = (ID: any) => {
        try {
            axios.post(`https://localhost:4000/ProjectArchives/moveProject/${ID}`,  {
                headers: { authorization: Cookie.auth_token }
            }) 
            .then(() => {
                window.location.reload()
